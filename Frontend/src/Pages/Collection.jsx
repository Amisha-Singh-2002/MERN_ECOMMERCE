...
const Collection = () => {
  ...
  const applyFilter =() =>{
    ...
    if(category.length>0){
      const catLower = category.map(c=>String(c).toLowerCase());
      productsCopy = productsCopy.filter(item=> item.category && catLower.includes(String(item.category).toLowerCase()))
    }
    if(subCategory.length>0){
      const subLower = subCategory.map(s=>String(s).toLowerCase());
      productsCopy = productsCopy.filter(item=> item.subCategory && subLower.includes(String(item.subCategory).toLowerCase()))
    }
